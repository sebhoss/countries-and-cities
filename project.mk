...
work/LoadHubCityStaging.sql: .tmp/city-codes-sentinel
	for file in ./work/city-codes-*; do while read -r name; do echo "INSERT INTO HubCityStaging (LoadDate, RecordSource, CityCode) VALUES (STR_TO_DATE('${TIMESTAMP}', '%Y-%m-%d %H:%i:%s'), 'unece.org', '$${name}');" >> work/LoadHubCityStaging.sql; done < $${file}; done


.tmp/city-codes-sentinel: work/country-codes.txt
	mkdir --parents $(@D)
	cat work/country-codes.txt | tr '[:upper:]' '[:lower:]' | xargs --no-run-if-empty --replace sh -c "curl --silent https://service.unece.org/trade/locode/{}.htm | htmlq 'body > table:nth-child(3)' 'td:nth-child(2)' --remove-nodes 'body > table:nth-child(3) > tbody:nth-child(1) > tr:nth-child(1)' --text | sed 's/\xC2\xA0//g' > work/city-codes-{}.txt"
	touch $@
